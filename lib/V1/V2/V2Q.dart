...
class _V2QState extends State<V2Q>with TickerProviderStateMixin , WidgetsBindingObserver {
    ...
    return Scaffold( backgroundColor: Colors.white,
        appBar: AppBar(backgroundColor: Colors.white,iconTheme: IconThemeData(color: Colors.black),
         // leading: Container(alignment: Alignment.center,child:Text(startTime.toString(), style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold,fontSize: 30), textAlign: TextAlign.center,),),
          title: Text(startTime.toString(), style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold,fontSize: 30), textAlign: TextAlign.center,), centerTitle: true,elevation: 0,
        ...
        body: SingleChildScrollView(
            child: Column(children: <Widget>[
              ...
              Container(child: Column(children: <Widget>[
                ...
                // ]))),
               // Container(margin: EdgeInsets.only(bottom: 5, left: 20, right: 20), child:Text('問題',style: TextStyle(color:Colors.blueGrey[500]),),),
             ...
             //   Container(margin: EdgeInsets.only(top:20,bottom: 5, left: 20, right: 20), child:Text('答え',style: TextStyle(color:Colors.blueGrey[500]),),),
                ...
                Container(margin: EdgeInsets.only(top: 30, left: 10, right: 10),
                    child: Row(children: <Widget>[
                      Expanded(
                          child: Container(height: 90, width: 90,
                            child: Visibility(visible: OK,
                              child: IconButton(onPressed:() {add();correct();getTime ();set();}, icon: Icon(LineIcons.circle,size: 80,))
                              // ElevatedButton(
                      ...
                      Expanded(child: Container(height: 90, width: 90, child: Visibility(visible: OK,
                        child:  IconButton(onPressed:() {itemco = itemco + 1;getTime ();set();}, icon: Icon(LineIcons.times,size: 80,))
                        // ElevatedButton(
                ...
                Container(width:double.infinity,child:Text('${item.length.toString().padLeft(0, '  ')}/${co.toString()}',style: TextStyle(fontSize: 10,color: Colors.grey),textAlign: TextAlign.center,),),
                Container(width:double.infinity,child:Text(time.toString() + "周目",style: TextStyle(fontSize: 10,color: Colors.grey),textAlign: TextAlign.center,),),
                Container(margin: EdgeInsets.all(10), alignment: Alignment.center, child: Text("答えまで"+CountTime.toString() + "秒", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold,fontSize: 10), textAlign: TextAlign.center,),),
 ...
 Future<void> getID () async {SharedPreferences prefs = await SharedPreferences.getInstance();ID =  prefs.getString("ID")!; boo = prefs.getBool("有料判定1")?? true;}
  ...
  void set (){setState(()  {
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (context) => V2Result(item1,item2,item3,item4,widget.name,widget.item,widget.All)),);}
  });}
